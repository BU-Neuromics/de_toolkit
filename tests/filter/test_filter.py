...
def filter_count_obj(request):
    ...
    sample_info = [samp_names, ["case"] * hsamp + ["cont"] * hsamp]
    sample_info = pandas.DataFrame(list(zip(*sample_info)), columns=("id", "cov"), index=samp_names)
    ...
    counts = pandas.DataFrame(counts, index=count_names, columns=samp_names)
                ...
                with NamedTemporaryFile("wt") as out_f:
                    main(
                        [
                            ...
                            "-o",
                            out_f.name,
                            "nonzero(cov[case]) == 0 and zero(cov[cont]) == 0",
                            counts_f.name,
                            cov_f.name,
                            f"--report-dir={d}",
                        ]
...
def test_parser(filter_count_obj):
    ...
    p = parse_filter_command(
        "(mean(cov[case]) >= 100 and mean(cov[cont]) >= 1000) or nonzero(all) == 0"
    )
    ...
    filter_count_obj.column_data.cov_with_underscore = [
        _ + "_o" for _ in filter_count_obj.column_data.cov_with_underscore
    ]
    ...
    filter_count_obj.column_data["cov_with.period"] = [
        _ + ".o" for _ in filter_count_obj.column_data["cov_with.period"]
    ]
