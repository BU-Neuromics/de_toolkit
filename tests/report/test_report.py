...
def test_detk_module_json(fake_module):
    ...
    with TemporaryDirectory() as d:
        ...
        DetkModuleJSON(fake_module, json_path=os.path.join(d, "fn.json")).write(indent=2)
...
def test_report_generate(tmp_path):
    ...
    for gz in (
        "all_mRNA_nonzero_raw_counts_trim.csv.gz",
        "all_mRNA_nonzero_norm_counts_trim.csv.gz",
    ):
        with (
            gzip.open(os.path.join(fixture_dir, gz), "rb") as f_in,
    ...
    for kind in ("raw", "norm"):
        _run_detk(
            [
                ...
                "-o",
                f"{kind}_summary_stats.csv",
                "--column-data=sample_info.csv",
            ],
            cwd,
        )
    _run_detk(
        [
            ...
            "-o",
            "norm_filtered.csv",
            "all_mRNA_nonzero_norm_counts_trim.csv",
        ],
        cwd,
    )
