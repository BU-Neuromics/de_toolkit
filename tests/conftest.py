...
def fake_huge_column_data(request):
    ...
    covs = [["sample", "category", "cont_cov", "const_cov"]] + list(
        zip(
            names,
...
def fake_counts_pandas_dataframe(fake_counts_csv):
    return pandas.read_csv(fake_counts_csv, index_col=0)
...
@pytest.fixture
def fake_counts_obj(fake_counts_csv, fake_column_data_csv, fake_logistic_design):

    return make_counts_obj(fake_counts_csv, fake_column_data_csv, fake_logistic_design)
...
def fake_counts_pandas_dataframe_with_zeros(fake_counts_csv_with_zeros):
    return pandas.read_csv(fake_counts_csv_with_zeros, index_col=0)
...
def fake_counts_obj_with_zeros(
    fake_counts_csv_with_zeros, fake_column_data_csv, fake_logistic_design
):

    return make_counts_obj(fake_counts_csv_with_zeros, fake_column_data_csv, fake_logistic_design)
...
def fake_big_counts_data():
    ...
    for i in range(1000):
        ...
        p = uniform(0.1, 0.3)
        data.append(
            [f"gene{i}", negative_binomial(n, p), negative_binomial(n, p), negative_binomial(n, p)]
        )
    return data
...
def fake_big_counts_pandas_dataframe(fake_big_counts_csv):
    return pandas.read_csv(fake_big_counts_csv, index_col=0)
...
@pytest.fixture
def fake_big_counts_obj(fake_big_counts_csv, fake_column_data_csv, fake_logistic_design):

    return make_counts_obj(fake_big_counts_csv, fake_column_data_csv, fake_logistic_design)
...
def fake_huge_counts_data():
    ...
    data = [["gene"] + list(string.ascii_lowercase)]
    for i in range(30000):
        n = randint(5, 40)
        p = uniform(0.1, 0.3)
        data.append(
            [f"gene{i}"] + [negative_binomial(n, p) for _ in range(len(string.ascii_lowercase))]
        )
    return data
...
@pytest.fixture
def fake_huge_counts_obj(fake_huge_counts_csv, fake_column_data_csv, fake_logistic_design):

    return make_counts_obj(fake_huge_counts_csv, fake_column_data_csv, fake_logistic_design)
...
def make_counts_obj(counts_csv, column_data_csv, design=None):
    from de_toolkit import CountMatrixFile

    counts_obj = CountMatrixFile(counts_csv, column_data_csv, design)
...
def fake_counts_gene_lengths(request, fake_counts_obj):
    return pandas.Series(
        range(2, len(fake_counts_obj.counts.index) + 2), index=fake_counts_obj.counts.index
    )
