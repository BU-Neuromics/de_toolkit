...
class RscriptExecutableNotFound(Exception):
    pass


class RPackageMissing(Exception):
    pass
...
def check_r_package(pkg):
    ...
    if r_path is None:
        ...
        return False
    cmd = [r_path, "-e", f"library({pkg})"]
    logger.debug("check_r_package cmd: %s", cmd)
    p = subprocess.run(cmd, shell=False, capture_output=True)
...
def require_r_package(pkg):
    ...
    if not check_r_package(pkg):
        raise RPackageMissing(
            f"R package {pkg} is needed for this "
        ...
        def decorator(*args, **kwargs):
            if not check_r():
                raise RscriptExecutableNotFound(
                    "Rscript executable could not be "
...
def require_deseq2(f):
    ...
    def _f(*args, **kwargs):
        if not check_deseq2():
            raise RPackageMissing(
                "R package DESeq2 is needed for this "
                "functionality. In R, try installing with:\n\n"
...
class WrapR:
    ...
    def __init__(
        ...
        if output_fn is None:
            self._files["output"] = open(os.path.join(self.routput_dir, "counts_out.csv"), "w")
        ...
        if metadata_out_fn is None:
            self._files["meta_out"] = open(os.path.join(self.routput_dir, "meta_out.csv"), "w")
    ...
    def execute(self):
        ...
        cmd = (
            (
                "{rpath} --vanilla {rscript} {counts_in} {meta_in} {params_in} "
                "{output} {meta_out} {params_out}"
            )
            .format(**self._paths)
            .split(" ")
        )
        ...
        if os.path.exists(self._paths["output"]):
            try:
                self.output = pd.read_csv(self._paths["output"], index_col=0)
        ...
        if os.path.exists(self._paths["meta_out"]):
            try:
                self.metadata_out = pd.read_csv(self._paths["meta_out"], index_col=0)
            except pd.errors.EmptyDataError:
...
def main(argv=sys.argv):
    ...
    if args["run"]:
        ...
        if args["<counts_in>"] is not None:
            try:
                counts_obj = CountMatrixFile(
                    args["<counts_in>"], args["--meta-in"], strict=args.get("--strict", False)
                )
        ...
        if not r:
            logger.error(
                RscriptExecutableNotFound(
                    "Rscript executable not found, wrapr interface and functions will not work"
                )
            )
            sys.exit(1)
        ...
        if not jsonlite:
            logger.error(
                RPackageMissing(
                    "ERROR: R package jsonlite must be installed, "
