...
logistf_test = pytest.mark.skipif(
    not check_r_package("logistf"), reason="logistf package not installed, skipping test"
)
...
def test_R_check_names_nonsense(deseq2_test_counts_obj):
    ...
    main(
        [
            "detk-de",
            "deseq2",
            "counts ~ RIN + Status[CNT]",
            "tests/de/R_check_names_counts.csv",
            "tests/de/R_check_names_meta.csv",
            "-o",
            out_fn,
        ]
    )
    ...
    counts_obj = CountMatrix(cnts, covs, "counts ~ cov[low] + category[control]")

    # full model results
    res = deseq2(counts_obj, normalized=True, all_coeff_results=True, routput_dir="tmp_output")
...
def deseq2_test_counts_obj():
    ...
    base_mean = 1000.0  # one group has a mean count of 1000
    ...
    base_counts = numpy.random.negative_binomial(base_r, base_p, size=(len(real_lfc), N1))
    ...
    with NamedTemporaryFile() as f:
        ...
        with NamedTemporaryFile() as counts_f:
            ...
            with NamedTemporaryFile() as covs_f:
                ...
                deseq2_test_counts_obj.column_data.to_csv(
                    fake_column_data_csv, index_label="sample"
                )
                ...
                main(
                    [
                        "detk-de",
                        "deseq2",
                        "counts ~ cont_cov + category[control]",
                        fake_counts_csv,
                        fake_column_data_csv,
                        "-o",
                        out_fn,
...
def test_deseq2_de(deseq2_test_counts_obj):
    ...
    # full model results
    res = deseq2(deseq2_test_counts_obj, normalized=True, all_coeff_results=True)
    ...
    res = deseq2(deseq2_test_counts_obj, normalized=True, all_coeff_results=False)
    # these counts have true lfc in range(-4,5)
    assert numpy.allclose(res["category__case__log2FoldChange"], numpy.arange(-4, 5), atol=0.2)

    # with cores
    res = deseq2(deseq2_test_counts_obj, normalized=True, all_coeff_results=True, cores=2)
    # these counts have true lfc in range(-4,5)
    assert numpy.allclose(res["category__case__log2FoldChange"], numpy.arange(-4, 5), atol=0.2)
    ...
    with pytest.raises(InvalidDesignException):
        res = deseq2(
            CountMatrix(deseq2_test_counts_obj.counts, deseq2_test_counts_obj.column_data, None)
        )
...
def test_firth_cli_w_cov(fake_counts_csv, fake_column_data_csv):
    main(
        ["detk-de", "firth", "category ~ cont_cov + counts", fake_counts_csv, fake_column_data_csv]
    )
...
def logistic_test_counts_obj():
    ...
    N2 = N - N1
    counts = pandas.DataFrame(
        columns=[f"gene{_}" for _ in range(N)], index=("gene1", "gene2", "gene3"), dtype="float64"
    )
    # null
    counts.loc["gene1"] = numpy.random.randint(100, size=N)
    # up in case
    counts.loc["gene2"] = numpy.concatenate(
        [numpy.random.normal(0.75, 1, size=N1), numpy.random.normal(0.25, 1, size=N2)]
    )
    # up in control
    counts.loc["gene3"] = numpy.concatenate(
        [numpy.random.normal(0.25, 1, size=N1), numpy.random.normal(0.75, 1, size=N2)]
    )
...
def logistic(counts_obj):
    ...
    coeffs = pandas.DataFrame(columns=exog.columns, index=counts_obj.counts.index, dtype="float64")
...
def test_firth_rda(logistic_test_counts_obj):
    ...
    # test with rda
    with NamedTemporaryFile() as f:
        firth_logistic_regression(logistic_test_counts_obj, rda=f.name)
        f.flush()
...
def test_firth_parallel(logistic_test_counts_obj):
    ...
    firth_logistic_regression(logistic_test_counts_obj, cores=2)
