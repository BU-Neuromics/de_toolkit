...
def build_pca(mod):
    ...
    tooltip = [{'field': 'sample', 'title': 'sample'},
    ...
    return _vl({
        ...
        'layer': [
            {'mark': {'type': 'area', 'opacity': 0.25, 'color': _CAT[0]},
             'encoding': {'x': {'field': 'pct', 'type': 'quantitative'},
                          'y': {'field': 'entropy', 'type': 'quantitative'}}},
            {'mark': {'type': 'line', 'color': _CAT[0]},
             'encoding': {'x': {'field': 'pct', 'type': 'quantitative', 'title': 'feature percentile'},
                          'y': {'field': 'entropy', 'type': 'quantitative', 'title': 'Shannon entropy'}}},
...
def build_colzero(mod):
    ...
    return _vl({
        ...
        'encoding': {
            ...
            'y': {'field': 'zero_frac', 'type': 'quantitative', 'title': 'fraction of zero counts',
                  'axis': {'format': '%'}},
            'tooltip': [{'field': 'sample'},
...
def build_rowzero(mod):
    rows = [{'num_zeros': z['num_zeros'], 'num_features': z['num_features'],
             'feature_frac': z['feature_frac']} for z in mod['properties']['zeros']]
    return _vl({
        'data': {'values': rows},
        'mark': {'type': 'bar', 'color': _CAT[0]},
        'encoding': {
            'x': {'field': 'num_zeros', 'type': 'ordinal', 'title': 'number of zero-count samples'},
            'y': {'field': 'num_features', 'type': 'quantitative', 'title': 'number of features'},
            'tooltip': [{'field': 'num_zeros', 'title': 'zeros'},
...
def build_coldist(mod):
    ...
    return _vl({
        ...
        'encoding': {
            'x': {'field': 'quantile', 'type': 'quantitative', 'title': 'quantile',
                  'axis': {'format': '%'}},
...
REGISTRY = {
    ...
    'coldist': ('stats', 'Column count distribution',
                'Per-sample count distribution shown as quantile curves (symlog scale).', build_coldist),
    'rowdist': ('stats', 'Row count distribution',
                'Per-feature count distributions across samples (sampled overlay).', build_rowdist),
    'colzero': ('stats', 'Zero counts by sample',
                'Fraction of features with exactly zero counts in each sample.', build_colzero),
    'rowzero': ('stats', 'Zero counts by feature',
                'How many features have a given number of zero-count samples.', build_rowzero),
    'entropy': ('stats', 'Feature entropy',
                'Shannon entropy across feature percentiles with an example low-entropy threshold.',
                build_entropy),
    'pca': ('stats', 'Principal component analysis',
            'Samples on the first two principal components, coloured by a covariate.', build_pca),
}
