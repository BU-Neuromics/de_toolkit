...
def fake_count_dist_pandas_dataframe(fake_count_coldist_csv):
    return pandas.read_csv(fake_count_coldist_csv, index_col=0)
...
@pytest.fixture
def fake_count_coldist_obj(fake_count_coldist_csv, fake_column_data_csv, fake_design):

    return pytest.make_counts_obj(fake_count_coldist_csv, fake_column_data_csv, fake_design)
...
def test_stats_coldist_pctVal(fake_count_coldist_obj):
    ...
    assert coldist_pct == [_ / 10.0 for _ in range(10)]
...
def test_stats_coldist_log_pctVal(fake_count_coldist_obj):
    ...
    coldist_true = 10 ** np.percentile(
        np.log10(fake_count_coldist_obj.counts.a + 1), np.arange(10) * 10
    )
    assert np.allclose(coldist_pctVal, coldist_true)
...
def test_stats_coldist_log_bins(fake_count_coldist_obj):
    ...
    true_dists, true_bins = np.histogram(np.log10(fake_count_coldist_obj.counts.a + 1), bins=2)
...
def test_stats_coldist_json(fake_count_coldist_obj):
    ...
    coldist_true = np.percentile(fake_count_coldist_obj.counts.a, np.arange(10) * 10)
    assert np.allclose(coldist_pctVal, coldist_true)
...
@pytest.fixture
def fake_count_rowdist_obj(fake_count_rowdist_csv, fake_column_data_rowdist_csv, fake_design):

    return pytest.make_counts_obj(fake_count_rowdist_csv, fake_column_data_rowdist_csv, fake_design)
...
def test_stats_rowdist_pct(fake_count_rowdist_obj):
    output = RowDist(fake_count_rowdist_obj, 20, False, False)
    row_pct = output["pct"]
    assert row_pct == list((_ + 1) / 0.2 for _ in range(20))
...
@pytest.fixture
def fake_counts_rowzero_obj(fake_counts_rowzero_csv, fake_column_data_csv, fake_design):

    return pytest.make_counts_obj(fake_counts_rowzero_csv, fake_column_data_csv, fake_design)
...
def test_stats_rowzero_numzero(fake_counts_rowzero_obj):
    ...
    assert stats["mean"] == [1, 0.8, 0.6, 0.4, 0.2, 0]
...
def test_stats_entropies(fake_counts_entropy_obj):
    ...
    assert np.allclose(
        entropies["pctVal"], np.percentile(true_entropies, true_pct, method="higher")
    )
...
def pca_counts_obj(request):
    ...
    counts = pandas.DataFrame(X, columns=("a", "b"), index=[f"gene{_}" for _ in range(X.shape[0])])
    col_data = pandas.DataFrame({"cond": ["c1", "c2"], "cov": [0, 1]}, index=counts.columns)

    return CountMatrix(counts, col_data)
